...
public class TurnToDeg extends Command {
	...
	// Array is used to determine the motor speed, ex. if mini has of less 180 and greater than 20 set speed to .5
	private final double miniP[][] = { { 180, .5 }, { 20, .3 }, { 10, .2 }, { 5, .15 }, { 3, .11 }, { 2, .08 },
			{ 1, .06 }, { 0, 0 } };
	private final double compP[][] = { { 180, .4 }, { 20, .3 }, { 10, .25 }, { 5, .22 }, { 3, .20 }, { 2, .19 },
			{ 1, .175 }, { .75, .170 },{ 0, .17 } };
	private double activeP[][] = miniP;

	// END AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=VARIABLE_DECLARATIONS

	// BEGIN AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=CONSTRUCTOR
	public TurnToDeg(double degrees, double timeout) {
	...
	public TurnToDeg(double degrees) { // Overloaded for 5 second time out
		this(degrees, 5);
	}
	...
	protected boolean isFinished() {
		double convTime = (new Date().getTime() - m_turnToInit.getTime()); //Coverge time
		...
		Robot.logf(
				...
				err, avgError.getAverage(), kError, RobotMap.driveSystemLeftFront.getEncPosition(),
	...
	protected double getOutput(double error) {
		int i = 0;
		while (activeP[i][0] > Math.abs(error)) {
			i++;
		}
