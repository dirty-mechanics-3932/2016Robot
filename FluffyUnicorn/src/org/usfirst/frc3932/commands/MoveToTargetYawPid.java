...
public class MoveToTargetYawPid extends Command {
	...
	protected boolean isFinished() {
		...
		if (state == State.FINALCOAST) {
			vision.getTarget();
			String s = String.format("LP:%.0f RP:%.0f LS:%.1f RS:%.1f LC:%.2f RC:%.2f",
			...
			Robot.logf("Final Wait %s yaw:%.2f wait:%d %s%n", visionParms(), yaw, waitUntilCount - count, s);
	...
	protected boolean targeting() {
		...
		if (turnTo != null) {
			...
			vision.getTarget(); // Update the vision parameters
			...
			Robot.logf("Start TurnTo %s vision update:%d %n", visionParms(), (new Date().getTime() - start.getTime()));
